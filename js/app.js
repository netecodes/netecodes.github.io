...
function scrollMagic() {
	...
	tl
		.to(intro_title, 0.7, {autoAlpha:1, y: 20, ease: Power1.easeOut}, 0.4)
		.to(intro_kicker, 1, {autoAlpha:1, y: 20})
		.from(navbar, 1, {x: 200})
	...
	var $skill_bar = $('.skill-bar'),
		...
		skillTl
			...
			.staggerFrom($skill_bar, 1, {width:0, ease: Power3.easeOut}, '-=1')
	...
	var $thumb_unit = $('.thumb-unit'),
		...
		workTl
			.staggerFrom($thumb_unit, 0.4, {autoAlpha: 0, rotation: -10, ease: Bounce.easeIn}, 0.2)
			.from($work_title, 1, {autoAlpha:0, y:-30, ease:Linear.easeOut}, '-=0.2')
